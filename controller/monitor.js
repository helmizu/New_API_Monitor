...
function monitor(SLACK_WEBHOOK_URL = '', URL = []) {
    ...
    URL.map(api => {
        ...
        new TestAPI(api.method, api.url, {
                type: api.type,
                data: data,
                expectStatus: parseInt(api.expectStatus),
                expectKey: api.expectKey
            })
...
function cekNow(SLACK_WEBHOOK_URL = '', URL = {}, cb) {
    ...
    const API = new TestAPI(URL.method, URL.url, {
        type: URL.type,
        data: data,
        expectStatus: parseInt(URL.expectStatus),
        expectKey: URL.expectKey
    })
